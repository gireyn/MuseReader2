...
static QString compatibleSoundFontFileName(const QString& fileName)
      {
      if (fileName == Ms::DEFAULT_SOUNDFONT)
            return Ms::LEGACY_DEFAULT_SOUNDFONT;
      if (fileName == Ms::LEGACY_DEFAULT_SOUNDFONT)
            return Ms::DEFAULT_SOUNDFONT;
      return QString();
      }

static QString findSoundFontPath(const QFileInfoList& files, const QString& fileName)
      {
...
static QString findCompatibleSoundFontPath(const QFileInfoList& files, const QString& requestedFileName)
      {
      QString path = findSoundFontPath(files, requestedFileName);
      if (!path.isEmpty())
            return path;

      const QString fallback = compatibleSoundFontFileName(requestedFileName);
      if (!fallback.isEmpty())
            return findSoundFontPath(files, fallback);
...
bool Fluid::loadSoundFonts(const QStringList& sl)
      ...
      for (int i = sl.size() - 1; i >= 0; --i) {
            ...
            path = findCompatibleSoundFontPath(l, fileName);
            if (path.isEmpty()) {
